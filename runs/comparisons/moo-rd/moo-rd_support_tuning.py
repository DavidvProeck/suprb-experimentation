...
def run(problem: str, job_id: str, rule_amount: int, filter_subpopulation: str,
    ...
    estimator = SupRB(
        rule_discovery=NSGA2(
            ...
            fitness_objs=[
                lambda r: r.error_,
                lambda r: -np.count_nonzero(r.match_set_), # Support calculation
            ],
            fitness_objs_labels=[
                "Error",
                "-Support"
    ...
    experiment_name = f'MOO-RD-Support-tuning j:{job_id} p:{problem}; r:{rule_amount}; f:{filter_subpopulation}; -e:{experience_calculation}' or study_name
