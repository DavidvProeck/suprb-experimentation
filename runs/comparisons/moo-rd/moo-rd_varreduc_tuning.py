...
from suprb.optimizer.rule.nsga2 import NSGA2VarianceReduction
...
def run(problem: str, job_id: str, rule_amount: int, filter_subpopulation: str,
    ...
    estimator = SupRB(
        rule_discovery=NSGA2VarianceReduction(
            ...
            fitness_objs=[
                lambda r: r.error_, # Information Gain objective is added internally
            ],
            fitness_objs_labels=[
                "Error",
            ],
    ...
    experiment_name = f'MOO-RD-VarianceReduction-tuning j:{job_id} p:{problem}; r:{rule_amount}; f:{filter_subpopulation}; -e:{experience_calculation}' or study_name
