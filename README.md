...
3) Clone the SupRB fork, developed as part of my bachelor's thesis (https://github.com/DavidvProeck/suprb), into **src/**, replacing the **suprb/** folder there.

## Running the isolated experiments:
1) Make sure that **src/suprb** is added to path, else run `export PYTHONPATH=<path2repo>/suprb-experimentation/src/suprb:$PYTHONPATH` 
2) Run `cd <path2repo>/suprb-experimentation`
3) Then run the experiment, e.g. `python runs/run_without_tuning/isolated_RD/nsga2_novelty_P.py`
...
The easiest (and most comfortable) to run the scripts to create the graphs in logging_output_scripts is to use the **one_time_run.py**. Here is how to use it:
